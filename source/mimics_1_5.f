...
C***********************************************************************
C*************     MIMICS Version 1.5 MODEL              ***************
C***********************************************************************
C*************           Master Program                  ***************
C***********************************************************************
C
      PROGRAM MIMICS_version_1_5
        save
C
C***********************************************************************
...
c
            IF(CALL_SUB(1,1)) CALL DIELECTRIC(EPSILONR,EPSILONRC)
C
c--------- compute ground specular reflectivity matrix and -------------
c---------         backscattering phase matrix             -------------
c                                                                       
            IF(CALL_SUB(2,1)) CALL GROUND_LAYER
C
...
c
            IF(CALL_SUB(3,1)) CALL TRUNK_LAYER(LOG_CONSTITUENT(1))
c
c--------- compute crown layer phase and extinction matrices  ----------
c
            IF(CALL_SUB(4,1)) CALL CROWN_LAYER(LOG_CROWN)
c
...
C************************************************************************
C               ADDED BY A. MONSIVAIS 12/12/2009	
C                  
            CALL SOLVE_BRANCH(LOG_CONSTITUENT(1),LOG_CROWN)
            CALL SOLVE_LEAF(LOG_CONSTITUENT(1),LOG_CROWN)
C************************************************************************
c
        ENDDO
c
