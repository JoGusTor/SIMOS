...
C***  ONE branch.        (secondary branch)                          ***
C*********************************************************************** 
c** updated 3-7-91 to force reciprocity to hold for resonant branches ** 
C*********************************************************************** 
...
C***                          EXP_MAT                                ***
C***                          PDF_BR2_SETUP                          ***
C***    Called functions:     PDF_BR2                                ***
C*********************************************************************** 
C
        SUBROUTINE BRANCH_PHASE2(P_BRANCH_2)
        save
C
C***********************************************************************
...
C       = WAVE NUMBER (1/M)
C
C   P_BRANCH_2(4,4,4) = Four 4x4 Phase Matrices of the branches
C   P_BRANCH_2(L,K,1) = Phase Matrix of ground-direct term
C   P_BRANCH_2(L,K,2) = Phase Matrix of crown-ground term
C   P_BRANCH_2(L,K,3) = Phase Matrix of ground-crown term
C   P_BRANCH_2(L,K,4) = Phase Matrix of direct-crown term
C
...
C                      direction
C
C   M_BRANCH_P2(2,2) = 2x2 complex M matrix in positive-going direction
C                      -- AVERAGE SCATTEING MATRIX TIMES 2*PI*N/k0
C   M_BRANCH_M2(2,2) = 2x2 complex M matrix in negative-going direction
C
...
C
C   EXP_KAPPA_BR_P2(4,4) = positive-going attenuation of the branches
C                         alone
C   EXP_KAPPA_BR_M2(4,4) = negative-going attenuation of the branches
C                         alone
C
...
C
C   PDF_BR2(THETAC) = real-valued P.D.F. function describing the
...
C
        ...
        REAL P_BRANCH_2(4,4,4),STOKES_MAT(4,4)
        REAL P_BRANCH_tmp_1(4,4,4), P_BRANCH_tmp_2(4,4,4)
        REAL P_BRANCH_tmp_3(4,4,4)
        REAL EXP_KAPPA_BR_P2(4,4),EXP_KAPPA_BR_M2(4,4)
        REAL PDF, PDF_BR2, NORM, WORK
C
        COMPLEX J, CWORK
        COMPLEX SMAT_AVG(2,2,2),M_BRANCH_P2(2,2),M_BRANCH_M2(2,2)
...
C
        COMMON /BR2_EXT/ EXP_KAPPA_BR_p2, EXP_KAPPA_BR_m2
...
C
        ...
        print*,' Seconadry branch, resonant = ',resonant
C
C***********************************************************************
...
C
        ...
        PHII = 0.0
C
        call pdf_br2_setup(n_theta_1,t_rad_start_1,t_rad_stop_1,
...
c
      do 150 jj=1,n_theta_1
       ...
       pdf = pdf_br2(thetac)
       IF(PDF.GT.0.0)THEN
        print*,' branch 2a thetac,pdf = ',thetac,pdf
C
c-----------------------------------------------------------------------
...
c
      do 170 jj=1,n_theta_2
       ...
       pdf = pdf_br2(thetac)
       IF(PDF.GT.0.0)THEN
        print*,' branch 2b thetac,pdf = ',thetac,pdf
C
c-----------------------------------------------------------------------
...
c
      do 200 jj=1,n_theta_3
       ...
       pdf = pdf_br2(thetac)
       IF(PDF.GT.0.0)THEN
        print*,' branch 2c thetac,pdf = ',thetac,pdf
C
c-----------------------------------------------------------------------
...
C
        DO 250 i=1,4
         DO 250 k=1,4
          DO 250 l=1,4
           P_branch_2(l,k,i) = WORK*(delta_p1_t1*P_branch_tmp_1(l,k,i) +
...
C
        DO 260 k=1,2
         DO 260 l=1,2
          M_BRANCH_P2(l,k) = CWORK*(delta_p1_t1*Smat_avg_tmp_1(l,k,1) +
     &                              delta_p2_t2*Smat_avg_tmp_2(l,k,1) +
     &                              delta_p3_t3*Smat_avg_tmp_3(l,k,1))
260     CONTINUE
        DO 270 k=1,2
         DO 270 l=1,2
          M_BRANCH_M2(l,k) = CWORK*(delta_p1_t1*Smat_avg_tmp_1(l,k,2) +
...
c-----------------------------------------------------------------------
        if(resonant)then
         DO k=1,4
          DO l=1,4
           P_branch_2(l,k,2) = 0.5*P_branch_2(l,k,2)
           P_branch_2(l,k,3) = 0.5*P_branch_2(l,k,3)
          enddo
         enddo
        endif
c-----------------------------------------------------------------------
