...
c*********************************************************************** 
C
        SUBROUTINE SOLVE_BRANCH(LOG_TRUNK,LOG_CROWN)
        save
C
c***********************************************************************
...
C
        INTEGER I, J, K
C
        REAL T_SIG0_branch(4,4)
        REAL SIG0_branch(4,4,N_SCAT_TERMS)
        REAL T_SIG0_dB_branch(4,4)
        REAL SIG0_dB_branch(4,4,N_SCAT_TERMS)
        REAL WORK
c
        ...
        REAL RDUMMAT(4,4)
        REAL BACKTERMS_branch(4,4,N_SCAT_TERMS), T_branch(4,4)
...
C
        ...
        COMMON /STOKES_MATS_branch/ T_branch, BACKTERMS_branch
...
C
        DO I=1,4
         DO J=1,4
          PHASE_PASS(I,J) = BR1_DENS*P_branch(I,J,1)
         ENDDO
        ENDDO
C
...
C
        DO I=1,4
         DO J=1,4
          PHASE_PASS(I,J) = BR1_DENS*P_branch(I,J,2)
         ENDDO
        ENDDO
C
...
C
        DO I=1,4
         DO J=1,4
          PHASE_PASS(I,J) = BR1_DENS*P_branch(I,J,3)
         ENDDO
        ENDDO
C
...
C
        DO I=1,4
         DO J=1,4
          PHASE_PASS(I,J) = BR1_DENS*P_branch(I,J,4)
         ENDDO
        ENDDO
C
...
c
        call flip_stokes(RDUMMAT1,RDUMMAT)
c
        CALL SET_TERM(RDUMMAT,BACKTERMS_branch,CTHETA,1)
c
        call phase_diff(RDUMMAT,phi_hv_vv,phi_vh_vv,phi_hh_vv)
...
c
        ...
        CALL SET_TERM(RDUMMAT2,BACKTERMS_branch,CTHETA,2)
c
...
c
        ...
        CALL SET_TERM(RDUMMAT2,BACKTERMS_branch,CTHETA,3)
c
        call phase_diff(RDUMMAT2,phi_hv_vv,phi_vh_vv,phi_hh_vv)
...
c
        call flip_stokes(RDUMMAT1,RDUMMAT)
c
        CALL SET_TERM(RDUMMAT,BACKTERMS_branch,CTHETA,4)
c
        call phase_diff(RDUMMAT,phi_hv_vv,phi_vh_vv,phi_hh_vv)
...
c
        ...
        CALL SET_TERM(RDUMMAT1,BACKTERMS_branch,CTHETA,5)
c
        call phase_diff(RDUMMAT1,phi_hv_vv,phi_vh_vv,phi_hh_vv)
...
c
        ...
        CALL SET_TERM(RDUMMAT1,BACKTERMS_branch,CTHETA,6)
c
        call phase_diff(RDUMMAT1,phi_hv_vv,phi_vh_vv,phi_hh_vv)
...
c       CALL SET_TERM(RDUMMAT2,BACKTERMS_branch,1.0,7)
       CALL SET_TERM(RDUMMAT2,BACKTERMS_branch,ctheta,7)
c
        call phase_diff(RDUMMAT2,phi_hv_vv,phi_vh_vv,phi_hh_vv)
...
C
        DO 500 K=1,7
         DO 500 I=1,4
          DO 500 J=1,4
            T_branch(J,I) = T_branch(J,I) + BACKTERMS_branch(J,I,K)
500     CONTINUE
c
        call phase_diff(T_branch,phi_hv_vv,phi_vh_vv,phi_hh_vv)
...
C
        DO 620 K=1, N_SCAT_TERMS
          DO 610 I=1,2
            DO 600 J=1,2
                SIG0_branch(J,I,K) = WORK*BACKTERMS_branch(J,I,K)
                IF(SIG0_branch(J,I,K) .GT. 0.0)THEN
                SIG0_dB_branch(J,I,K) = 10.0*ALOG10(SIG0_branch(J,I,K))
                ELSE
                    SIG0_dB_branch(J,I,K) = -999.99
                ENDIF
600         CONTINUE
610       CONTINUE
620     CONTINUE
C
        DO 640 I=1,2
            DO 630 J=1,2
                T_SIG0_branch(J,I) = WORK*T_branch(J,I)
                IF(T_SIG0_branch(J,I) .GT. 0.0)THEN
                T_SIG0_dB_branch(J,I) = 10.0*ALOG10(T_SIG0_branch(J,I))
                ELSE
                    T_SIG0_dB_branch(J,I) = -999.99
                ENDIF
630         CONTINUE
640     CONTINUE
C

       open(unit=8,file='../output_dir/forest_sigma_branch_like.out')
       open(unit=7,file='../output_dir/forest_sigma_branch_cross.out')
C
       char='    '
       char2='    ' 
...
C
       ...
       WRITE(8,800) 
     &                T_SIG0_dB_branch(1,1),T_SIG0_dB_branch(2,2),
     &               (SIG0_dB_branch(1,1,K), 
     &               SIG0_dB_branch(2,2,K),K=1,N_SCAT_TERMS)
C
        WRITE(7,800) 
     &                T_SIG0_dB_branch(1,2),T_SIG0_dB_branch(2,1),
     &               (SIG0_dB_branch(1,2,K), 
     &                SIG0_dB_branch(2,1,K),K=1,N_SCAT_TERMS)
...
Cc*********************************************************************** 
CC
C        SUBROUTINE SET_TERM(INPUT,BACKTERMS_branch,MU0,K)
C        save
CC
Cc***********************************************************************
...
CC
C        INTEGER I,J,K
C        REAL MU0, BACKTERMS_branch(4,4,N_SCAT_TERMS),INPUT(4,4)
C
CC***********************************************************************
CC
C        DO 10 I=1,4
C          DO 10 J=1,4
C            BACKTERMS_branch(J,I,K) = INPUT(J,I)/MU0
C10      CONTINUE
CC
C        RETURN
C        END
